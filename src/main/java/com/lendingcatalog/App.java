...
public class App {
    ...
    public static void main(String[] args) throws FileNotFoundException, FileStorageException {
    ...
    private void initialize() throws FileStorageException, FileNotFoundException {
        // Requirement: Data transformation
        String filePath;
        File fileToRead = new File(FILE_BASE_PATH + "members.dat");
        try (Scanner fileReader = new Scanner(fileToRead)) {
            List<String> fileOutput = new ArrayList<>();
            while (fileReader.hasNext()) {
                String currentLine = fileReader.nextLine();
                String[] memberEntry = currentLine.split(FIELD_DELIMITER);
                Member member = new Member(memberEntry[0] , memberEntry[1]);
                String memberFilePath = memberEntry[2];
                fileOutput = FileStorageService.readContentsOfFile(memberFilePath);
                for (String entry : fileOutput) {
                    List<CatalogItem> newItem = new ArrayList<CatalogItem>();
                    newItem = entry.split(FIELD_DELIMITER);
                    catalog.put(Member.toString() , newItem);
                }
            }
        }
        catch (FileStorageException e) {
                throw new FileStorageException("Unable to open file.");
            }
